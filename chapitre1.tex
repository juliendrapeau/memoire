...
Un problème SAT est décrit par une formule propositionnelle. Résoudre le problème consiste à déterminer s'il existe une combinaison de variables dont l'évaluation de la formul
...
Dans l'étude du problème SAT, les formules propositionnelles sont souvent exprimées en \textit{forme normale conjonctive} (« Conjunctive Normal Form ») (CNF). On parle de formules CNF. Celle-ci consiste en une conjonction d'une ou plusieurs \textit{clauses}, où une clause est une disjonction d'un ou plusieurs littéraux. 

\begin{example}{Satisfaisabilité booléenne}{sat}
    Soit la formule CNF
    
    \begin{equation}
        \varphi(x_{1}, x_{2}, x_{3}, x_{4}) = (\neg x_{1} \lor x_{5} \lor x_{3}) \land (x_{2} \lor x_{4} \lor x_{5})
    \end{equation}
...
\begin{maindefinition}{Problème Pas-Tous-Égaux 3SAT}{}
